...
const users = (app, db) => {
    ...
    app.post('/login', async (req, res) => {
        ...
        passport.authenticate('local', { session: false }, (err, user) => {
            if (err || !user) {
                return res.status(422).json({ error: { message: 'Adresse e-mail ou mot de passe incorrect' } })
            }

            req.login(user, { session: false }, (err) => {
                if (err) {
                    res.status(422).json({ error: { message: 'Adresse e-mail ou mot de passe incorrect' } })
                }
                delete user.pass;
                const { _id } = user
                const data = { _id: _id }
